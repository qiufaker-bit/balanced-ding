(function (root, factory) {
  ...
  const SKILL_PERIOD = TERRAIN_CYCLE * 4;
  ...
  class MountainRvBalanceGame {
    ...
    update(dt) {
      ...
      if (floorContact > 0 && this.lifeGraceTimer <= 0) {
        this.fallMeter += (0.75 + floorContact * 1.4) * dt;
      } else {
        this.fallMeter = Math.max(0, this.fallMeter - dt * (this.lifeGraceTimer > 0 ? 2.6 : 0.7));
      }
    ...
    ensureCoins() {
      ...
      for (let x = start; x < end; x += 140) {
        ...
        const lane = key % 4;
        const lift = bridge > 0 ? 78 + lane * 10 : 52 + lane * 12;
        this.coins.push({
          key,
          x,
          y: this.roadY(x) - lift,
    ...
    ensureTeeth() {
      const startCycle = Math.floor((this.distance - 260) / (TERRAIN_CYCLE * 2));
      const endCycle = Math.ceil((this.distance + 1800) / (TERRAIN_CYCLE * 2));
      for (let cycle = startCycle; cycle <= endCycle; cycle += 1) {
        const key = cycle;
        if (this.teeth.some((tooth) => tooth.key === key)) {
          continue;
        }
        const x = cycle * TERRAIN_CYCLE * 2 + 1180;
        const bridge = this.bridgeFactor(x);
    ...
    updateCamera(dt) {
      ...
      const target = clamp(focusY - 325, -220, 260);
    ...
    terrainY(x) {
      ...
      const crest = t < 180 ? Math.sin(t / 180 * Math.PI) * 20 : 0;
      const lip = t > 220 && t < 360 ? smoothStep((t - 220) / 140) * 42 : 0;
      const drop = t >= 360 && t < 520 ? 52 * (1 - smoothStep((t - 360) / 160)) : 0;
      const pothole = t > 620 && t < 760 ? -Math.sin((t - 620) / 140 * Math.PI) * 30 : 0;
      const kick = t > 790 && t < 930 ? Math.sin((t - 790) / 140 * Math.PI) * 38 : 0;
      return BASELINE_Y
        + Math.sin(x * 0.0047) * 34 * d
        + Math.sin(x * 0.012 + 1.8) * 16 * d
        + Math.sin(x * 0.024) * 4 * d
        + crest
        + lip
        + drop
        + pothole
